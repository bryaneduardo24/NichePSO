...
###############################################################################
#N => Size of population, F => Mutation in range (0,2), CR => Recombination in range (0,1)
def differential_evolution(num_particles, bounds, x, v, u, fitness_function,err_best_g, pos_best_g, th, d, f, cr, maxiter):
    it=0

    while stop_condition(it,maxiter):
        v=mutation(x,v,f)
        u=recombination(x,v,u,cr,d)
        ...
        x=selection(x,u,th,fitness_function)
...
def initialization(x,x_min,x_max):
    for i in range(len(x)):
        for j in range(len(x[i])):
            x[i][j]=random.uniform(x_min,x_max)
    return x
...
def mutation(x,v,f):
    ...
    for i in range(len_x):
        ...
        for j in range(3):
            add_r(len_x,r)
        v[i].position_i[1]=float(x[r[0]].position_i[1])+f*(float(x[r[1]].position_i[1]) - float(x[r[2]].position_i[1]))
    return v


def recombination(x,v,u,cr,d):
#    import ipdb; ipdb.set_trace()
    for i in range(len(x)):
        u[i]=x[i]
        j=int(math.floor(random.uniform(0,d)))
        if random.uniform(0,1)<=cr:
            u[i].position_i[j]=v[i].position_i[j]
    return u


def selection(x,u,th,fitness_function):
    for i in range(len(x)):
...
if __name__ == "__main__":
    ...
    n_particles_niche = 40  # 40 65 130 260
    th_bisection = 1e-6
        ...
        for nth_ in new_route:
            for k in range(num_reserved_niches):
                ...
                if len(rn) > 0:
                    ...
                    #lst_particles, err_best_g, pos_best_g = differential_evolution(num_particles, bounds, lst_particles, lst_particles_v, lst_particles_u, fitness,err_best_g, pos_best_g, nth_, num_dimensions,f=0.8,cr=0.7,maxiter=30)
                    ...
                    reserved_niches_err, reserved_niches = remove_repeat(reserved_niches_err, reserved_niches, step_theta_=0.0005)  # 0.00018 # 0.0005
                    reserved_niches_err, reserved_niches = filter_to_minimal(reserved_niches_err, reserved_niches, 0.0045)  # 0.00018 # 0.0045
